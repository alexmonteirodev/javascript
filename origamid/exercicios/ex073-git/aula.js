...
//workflow
// 1- abre o projeto (main) e faz o pull pra atualizar
// 2- abre uma branch para trabalhar (git checkout -b 'x')
// 3- na branch, roda o projeto (npm run dev)
// 4- ao terminar de codar, da um cntrl c pra fechar o modo dev
// 5- da um run build pra compilar o código
// 6- adiciona as alterações (git add -A)
// 7- faz o comit dizendo o que foi feito ( git commit -m 'foi feito x')
// 8- volta na main pra conferir se alguem atualizou algo (git checkout main), e da um git pull
// 9- volta na branch que estava (git checkout x)
// 10- faz a união da branch com main (git merge main)
// 11- sobe as informações (git push) (obs: o terminal vai gerar o codigo do push e usa ele novamente)
// 12- depois disso aprova o merge no github
// 13- no codigo, volta pra main (git main)
// 14- puxa a atualização do código (git pull)
// 15- deleta a branch (git branch -d 'x')
// 16- limpa o terminal pra começar outro processo (clear)
